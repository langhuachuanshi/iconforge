...
// 提示词模式：guide 引导式（点选）/ mascot 吉祥物（ip-as-logo skill）/ expert 专家式（直接输入完整 prompt）
type PromptMode = "guide" | "mascot" | "expert";
...
// 空值会让色块变透明、提示词拼出 "null"。颜色在语义上必须有值：仅接受非空更新。
function nonNullColor(set: (v: string) => void) {
  return (v: string | null) => {
    if (v) set(v);
  };
}
const setC1 = nonNullColor((v) => {
  mascotC1.value = v;
});
const setC2 = nonNullColor((v) => {
  mascotC2.value = v;
});
const setMascotBg = nonNullColor((v) => {
  mascotBg.value = v;
});
const setSolid = nonNullColor((v) => {
  solidColor.value = v;
});
const setGradA = nonNullColor((v) => {
  gradientCustomA.value = v;
});
const setGradB = nonNullColor((v) => {
  gradientCustomB.value = v;
});
...
const extraSegment = computed(() => {
  ...
  else if (bgMode.value === "solid" && solidColor.value)
    parts.push(`solid ${solidColor.value} background`);
  else if (bgMode.value === "gradient" && gradientFinal.value)
    parts.push(`${gradientFinal.value} gradient background`);
  else if (bgMode.value === "custom" && bgCustomText.value.trim())
    parts.push(bgCustomText.value.trim());
  // 细节标签 → 英文
  if (detailTags.value.length) {
    const ens = detailTags.value
      .map((label) => DETAIL_OPTIONS.find((o) => o.label === label)?.en)
      .filter(Boolean);
...
const finalPrompt = computed(() => {
  // 吉祥物模式：预览显示第一张（各张仅"出现角落"一行不同，张数见下方提示行）
  if (promptMode.value === "mascot") {
    return mascotPrompts.value[0] ?? "...";
  }
  ...
  const prefix =
    currentTemplate.value?.promptPrefix?.replace(
      "{concept}",
      concept.value || "...",
    ) ?? "";
  let p = prefix;
  if (extraSegment.value) {
    p += ". ";
    p += extraSegment.value;
  }
...
async function handleGenerate() {
  // 校验：吉祥物查主体，专家模式查 expertPrompt，引导式查 concept
  if (promptMode.value === "mascot") {
    if (!mascotSubject.value.trim()) {
      ElMessage.warning("请输入 IP 主体");
      return;
    }
  } else if (promptMode.value === "expert") {
  ...
  const baseParams = {
    ...
    negativePrompt: isExpert
      ? undefined
      : negativePrompt.value.trim() || undefined,
    rawPrompt: isMascot
      ? mascotList![0]
      : isExpert
        ? expertPrompt.value.trim()
        : undefined,
  };
  ...
  const n = mascotList
    ? mascotList.length
    : Math.min(Math.max(genCount.value, 1), 2);
  ...
  async function generateWithRetry(
    ...
    retries = 2,
  ): Promise<{ b64: string; iconId: string }> {
    ...
    for (let i = 0; i < n; i++) {
      ...
      try {
        const r = await generateWithRetry(
          mascotList ? { ...baseParams, rawPrompt: mascotList[i] } : baseParams,
        );
  ...
  <div class="gen-root" v-loading="loadingMeta">
    ...
    <div class="gen-body">
      ...
      <div
        ...
        :element-loading-text="genLoadingText"
      >
        <!-- 单图：自适应撑满，保持正方形 -->
        <div
        ...
        <div
          ...
          class="result-grid"
          :class="`cols-${Math.min(results.length, 4)}`"
        >
          <div
        ...
        <div v-if="results.length" class="canvas-actions">
          <span class="action-hint"
            >已选中第 {{ selectedIdx + 1 }} 张，点击图片可切换</span
          >
      ...
      <div class="side-panel">
        <el-card>
          <el-form label-position="top" size="default">
            ...
            <template v-if="promptMode === 'mascot'">
              <div class="step"><span class="step-num">1</span> IP 主体</div>
              <el-form-item>
                <el-input
                  v-model="mascotSubject"
                  placeholder="例如：橘猫、小机器人、水滴精灵、太空狐…"
                  maxlength="100"
                />
              </el-form-item>

              <div class="step">
                <span class="step-num">2</span> 定义特征
                <span class="label-hint">— 可选，最多一个</span>
              </div>
              <el-form-item>
                <el-input
                  v-model="mascotFeature"
                  placeholder="例如：一对大圆耳朵、螺旋角、宽面罩…留空由 AI 决定"
                  maxlength="100"
                />
              </el-form-item>

              <div class="step">
                <span class="step-num">3</span> 配色
                <span class="label-hint">— 三色语义：主色① + 主色② + 背景</span>
              </div>
              <el-form-item>
                <el-radio-group v-model="mascotColorMode" size="small">
                  <el-radio-button value="auto">AI 自动</el-radio-button>
                  <el-radio-button value="custom">自定义三色</el-radio-button>
                </el-radio-group>
                <div
                  v-if="mascotColorMode === 'custom'"
                  class="gradient-pickers"
                  style="margin-top: 8px"
                >
                  <el-color-picker
                    :model-value="mascotC1"
                    size="small"
                    @update:model-value="setC1"
                  />
                  <span class="grad-arrow">+</span>
                  <el-color-picker
                    :model-value="mascotC2"
                    size="small"
                    @update:model-value="setC2"
                  />
                  <span class="grad-arrow">/</span>
                  <el-color-picker
                    :model-value="mascotBg"
                    size="small"
                    @update:model-value="setMascotBg"
                  />
                  <span class="tool-desc">主色① + 主色② / 背景</span>
                </div>
              </el-form-item>
              ...
              <el-form-item>
                <el-input
                  ...
                  type="textarea"
                  :rows="2"
                  placeholder="还想补充什么？例如：气质更安静一点、腮红明显一点…"
                  maxlength="300"
                />
              </el-form-item>

              <div class="step">
                <span class="step-num">6</span> 负向提示词
                <span class="label-hint">— 可选</span>
              </div>
              <el-form-item>
                <el-input
                  ...
                  type="textarea"
                  :rows="2"
                  placeholder="不希望出现的内容，如：文字、模糊、变形"
                  maxlength="300"
                />
              </el-form-item>
            </template>
            ...
            <!-- 专家模式：完整提示词输入 -->
            <template v-else-if="promptMode === 'expert'">
              <el-form-item>
            ...
            <template v-else>
              ...
              <el-form-item>
                ...
                <div v-if="bgMode === 'gradient'" class="gradient-custom">
                  ...
                  <div class="gradient-pickers">
                    <el-color-picker
                      :model-value="gradientCustomA"
                      size="small"
                      @update:model-value="setGradA"
                    />
                    <span class="grad-arrow">→</span>
                    <el-color-picker
                      :model-value="gradientCustomB"
                      size="small"
                      @update:model-value="setGradB"
                    />
                    <div
                      class="gradient-preview"
                      :style="{
                        background: `linear-gradient(135deg, ${gradientCustomA}, ${gradientCustomB})`,
                      }"
                    />
                  </div>
                </div>
              ...
              <el-form-item>
                ...
                <el-input
                  v-model="detailCustom"
                  placeholder="或输入自定义细节，如：白色文字、数字 090..."
                  maxlength="200"
                  style="margin-top: 8px"
                />
              </el-form-item>
              ...
              <!-- Step 5: 补充指令 -->
              <div class="step">
                <span class="step-num">5</span> 补充指令
                <span class="label-hint">— 可选</span>
              </div>
              <el-form-item>
                <el-input
                  ...
                  type="textarea"
                  :rows="2"
                  placeholder="还想补充什么？例如：参考 iOS 18 风格..."
                  maxlength="300"
                />
              </el-form-item>
            ...
            <el-collapse v-model="previewOpenNames">
              <el-collapse-item title="查看完整提示词" name="prompt">
                ...
                <p
                  v-if="promptMode !== 'expert' && negativePrompt.trim()"
                  class="neg-preview"
                >
            ...
            >
              {{ generating ? genBtnText : genBtnIdleText }}
            </el-button>
          </el-form>
        </el-card>
      </div>
...
.prompt-input-wrap .copy-fab {
  bottom: 28px;
}
.prompt-preview-wrap .copy-fab {
  bottom: 6px;
}
