...
- **生成图标** —— AI 文生图，支持通义万相（阿里云百炼）、字节豆包 Seedream、智谱 CogView，自带 14 个图标风格模板。三种模式：引导式点选、吉祥物（ip-as-logo 三色/角落构图批量候选）、专家式直输（原样发送，不追加任何词），可一键复制完整提示词
- **编辑图标** —— PS 风格左侧工具栏 + 右侧抽屉配置面板
  ...
  - 形状遮罩：圆角矩形（比例预览）、圆形
  - 调色：亮度、对比度、饱和度
  - 智能擦除：涂抹/框选擦除水印、文字、杂物（LaMa 本地修复，离线免费；支持导入自定义 ONNX）
